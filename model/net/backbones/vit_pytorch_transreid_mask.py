...
class PatchEmbed_overlap(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
        ...
        self.sem_pool = nn.MaxPool2d(patch_size, stride_size)
    ...
    def forward(self, x, sem):
        ...
        x = self.proj(x)
        if not sem is None:
            sem = self.sem_pool(sem)    # B, S, H', W'
            sem = sem.flatten(2).transpose(1, 2)    # B, P, S
        x = x.flatten(2).transpose(1, 2) # [64, P, 768]
        return x, sem
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, sem, camera_id, view_id):
        B = x.shape[0]
        x, sem = self.patch_embed(x, sem)
    ...
    def forward(self, x, sem, cam_label=None, view_label=None):
        x, sem = self.forward_features(x, sem, cam_label, view_label)
        return x, sem
