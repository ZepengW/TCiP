...
def main():
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(dataloader)):
            ...
            parsing_result_path = img_path.replace('.jpg','.png').replace('test','test-mask').replace('train','train-mask').replace('val','val-mask')
            parsing_result_dir = os.path.dirname(parsing_result_path)
            if not os.path.isdir(parsing_result_dir):
                os.makedirs(parsing_result_dir)
